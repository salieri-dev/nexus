...
@Client.on_message(filters.command(["altgirls"]), group=2)
...
async def woman_command(client: Client, message: Message):
    ...
    try:
        ...
        total_images = sum(len([f for f in files if f.lower().endswith((".png", ".jpg", ".jpeg", ".gif"))]) for _, _, files in os.walk(folder_path))
        ...
        if not is_private:
            ...
            for i, image_path in enumerate(image_paths, 1):
                ...
                if all_members:
                    ...
                    verb_suffix = "пользователя" if any(verb in random_term for verb in ACCUSATIVE_VERBS) else "пользователю"
