...
class ThxCumService:
    ...
    async def process_image(
        self,
        input_source: Union[str, bytes, BytesIO],
        ...
        try:
            ...
            if isinstance(input_source, str):
                input_to_paste = await asyncio.get_event_loop().run_in_executor(
                    self.thread_pool,
                    self._load_image_cv2,
                    input_source
                )
            ...
            input_to_paste = await asyncio.get_event_loop().run_in_executor(
                self.thread_pool,
                self._resize_maintaining_aspect,
                input_to_paste,
                max_width,
                max_height
            )
            ...
            composition = await asyncio.get_event_loop().run_in_executor(
                self.thread_pool,
                self._add_film_grain,
                composition
            )
            ...
            final_image = await asyncio.get_event_loop().run_in_executor(
                self.thread_pool,
                self._create_final_composition,
                transformed
            )
