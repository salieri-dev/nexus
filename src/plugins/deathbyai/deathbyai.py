...
async def start_game_command(client: Client, message: Message):
    ...
    try:
        ...
        # Start new game
        game = await DeathByAIService.start_game(chat_id=message.chat.id, message_id=status_msg.id, initiator_id=message.from_user.id)
        ...
        message_text, keyboard = DeathByAIService.format_game_message(game)
        await status_msg.edit_text(message_text, reply_markup=keyboard)

        # Start timer update loop
        try:
            while DeathByAIService.get_remaining_time(game) > 0:
            ...
            if game and game["status"] == "active":
                try:
                    ...
                    final_game = await DeathByAIService.end_game(chat_id=message.chat.id)
                    if final_game:
                        # Send results as new message
                        results = DeathByAIService.format_results(final_game)
                        results_msg = await message.reply_text(results, quote=False)

                        try:
                            # Update original message with game state without button
                            message_text, _ = DeathByAIService.format_game_message(final_game, show_button=False)
                        ...
                        try:
                            ...
                            end_message = DeathByAIService.format_end_message(final_game, results_msg.id)
...
async def handle_strategy(client: Client, message: Message):
    ...
    try:
        ...
        if not await DeathByAIService.validate_game_message(message_id=message.reply_to_message.id, reply_message_id=message.reply_to_message.id):
            return
