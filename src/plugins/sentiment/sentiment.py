...
@command_handler(
    commands=["sentiment"],
    description="Анализирует настроение в чате",
    group="Аналитика"
)
@Client.on_message(filters.command(["sentiment"]), group=1)
@rate_limit(
    operation="sentiment_handler",
    window_seconds=2,
    on_rate_limited=lambda message: message.reply("🕒 Подождите 2 секунды перед следующим запросом!")
)
    ...
    try:
        ...
        if messages:
            ...
            await message.reply_photo(
                photo=graph_bytes,
                caption=MESSAGES["SENTIMENT_GRAPH_CAPTION"],
                quote=True
            )
