...
class SummaryRepository:
    ...
    async def get_summaries_by_chat(
        ...
        """
        cursor = self.summaries.find({"chat_id": chat_id}) \
            .sort("generated_at", -1) \
            .skip(skip) \
            .limit(limit)
        ...
        pipeline = [
            {
                "$group": {
                    ...
                    "total_chats": {"$addToSet": "$chat_id"},
                    "avg_themes_per_summary": {"$avg": {"$size": "$themes"}},
