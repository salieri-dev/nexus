...
class ImagegenModelRepository:
    ...
    async def update_lora(self, id: str, updates: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            result = await self.loras_collection.update_one(
                {"id": id},
                {"$set": updates}
            )
...
PeerConfigModel.register_param(
    param_name="imagegen_cfg",
    param_type="plugin:imagegen",
    default=DEFAULT_CONFIG.copy(),
    description="Настройки генерации изображений",
    display_name="Настройки генерации изображений",
