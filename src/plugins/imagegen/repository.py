...
__all__ = ["ImagegenRepository", "ImagegenModelRepository", "ImagegenRequestRepository"]
...
class ImagegenRequestRepository:
    """Repository for handling imagegen request history."""

    def __init__(self):
        """Initialize the repository with database client."""
        self.db_client = DatabaseClient.get_instance()
        self.db = self.db_client.db
        self.requests_collection = self.db["imagegen_requests"]
    ...
    async def initialize(self):
        ...
        try:
            ...
            log.info("ImagegenRequestRepository initialized successfully")
        except Exception as e:
            log.error("Error initializing ImagegenRequestRepository", error=str(e))

    async def create_request(self, user_id: int, chat_id: int, prompt: str, config: Dict[str, Any],
                           payload: Dict[str, Any], status: str = "processing") -> Dict[str, Any]:
        """
        Create a new image generation request record.
        ...
        try:
            ...
            log.info("Created new imagegen request", user_id=user_id, prompt=prompt, status=status)
            return request
        except Exception as e:
            log.error("Error creating imagegen request", error=str(e), user_id=user_id, prompt=prompt)
            return None
    ...
    async def update_request(self, request_id: str, image_urls: List[str] = None,
        ...
        try:
            ...
            log.info("Updated imagegen request", request_id=request_id, status=status)
            return updated_request
        except Exception as e:
            log.error("Error updating imagegen request", error=str(e), request_id=request_id)
            return None
            
    # Keeping the old method name for backward compatibility, but with updated implementation
    async def save_request(self, user_id: int, chat_id: int, prompt: str, config: Dict[str, Any],
        ...
        try:
            ...
            if status == "processing" or not image_urls:
                ...
                return await self.create_request(user_id, chat_id, prompt, config, payload, status)
            else:
            ...
            log.error("Error in save_request", error=str(e), user_id=user_id, prompt=prompt)
            return None
        except Exception as e:
            log.error("Error saving imagegen request", error=str(e), user_id=user_id, prompt=prompt)
            return None
    ...
    async def get_user_requests(self, user_id: int, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        """
        try:
            cursor = self.requests_collection.find({"user_id": user_id}).sort("timestamp", -1).limit(limit)
            requests = await cursor.to_list(length=limit)
            return requests
        except Exception as e:
            log.error("Error getting user imagegen requests", error=str(e), user_id=user_id)
            return []
    ...
    async def get_chat_requests(self, chat_id: int, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        """
        try:
            cursor = self.requests_collection.find({"chat_id": chat_id}).sort("timestamp", -1).limit(limit)
            requests = await cursor.to_list(length=limit)
            return requests
        except Exception as e:
            log.error("Error getting chat imagegen requests", error=str(e), chat_id=chat_id)
            return []
    ...
    async def get_recent_requests(self, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        """
        try:
            cursor = self.requests_collection.find().sort("timestamp", -1).limit(limit)
            requests = await cursor.to_list(length=limit)
            return requests
        except Exception as e:
            log.error("Error getting recent imagegen requests", error=str(e))
            return []
