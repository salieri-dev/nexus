...
from .constants import (
    CALLBACK_PREFIX, MODEL_CALLBACK, NEGATIVE_PROMPT_CALLBACK, CFG_SCALE_CALLBACK,
    LORAS_CALLBACK, SCHEDULER_CALLBACK, IMAGE_SIZE_CALLBACK, BACK_CALLBACK,
    AVAILABLE_MODELS, AVAILABLE_LORAS, AVAILABLE_SCHEDULERS, IMAGE_SIZES, load_models_and_loras
)
...
async def create_settings_keyboard(config: Dict[str, Any]) -> InlineKeyboardMarkup:
    ...
    current_model = next((name for name, value in AVAILABLE_MODELS.items()
                         if value == config.get("model")), "Не выбрано")
    
    current_scheduler = next((name for name, value in AVAILABLE_SCHEDULERS.items()
                             if value == config.get("scheduler")), "Не выбрано")
    ...
    try:
        ...
        if len(message.command) > 1:
            ...
            await message.reply(
                ...
                "или выберите параметр для настройки:",
...
async def handle_imagegen_callback(client: Client, callback_query: CallbackQuery):
    ...
    try:
        ...
        if data == NEGATIVE_PROMPT_CALLBACK:
            ...
            await callback_query.edit_message_text(
                "🚫 **Негативный промпт**\n\n"
                f"Текущее значение: `{config.get('negative_prompt', '')}`\n\n"
        ...
        if data == CFG_SCALE_CALLBACK:
            ...
            await callback_query.edit_message_text(
                "⚙️ **CFG Scale**\n\n"
                f"Текущее значение: `{config.get('cfg_scale', 7.0)}`\n\n"
        ...
        if data.startswith(LORAS_CALLBACK):
            ...
            if lora_id == "list":
                ...
                await callback_query.edit_message_text(
                    "🧩 **Выберите Loras:**\n\n"
                    "Нажмите на Lora для выбора/отмены выбора",
                    reply_markup=keyboard,
                    parse_mode=ParseMode.MARKDOWN
                )
            else:
        ...
        if data.startswith(IMAGE_SIZE_CALLBACK):
            ...
            if size_id == "list":
                ...
                await callback_query.edit_message_text(
                    "⚙️ **Настройки генерации изображений**\n\n"
                    f"✅ Размер изображения успешно изменен\n\n"
                    "Выберите параметр для настройки:",
                    reply_markup=keyboard,
                    parse_mode=ParseMode.MARKDOWN
                )
...
async def handle_imagegen_text_input(client: Client, message: Message):
    ...
    try:
        ...
        if "Негативный промпт" in replied_text and "Отправьте новый негативный промпт" in replied_text:
            ...
            await message.reply(
                "⚙️ **Настройки генерации изображений**\n\n"
                f"✅ Негативный промпт успешно изменен\n\n"
                "Выберите параметр для настройки:",
                reply_markup=keyboard,
                parse_mode=ParseMode.MARKDOWN
            )
    ...
    try:
        ...
        if len(command_parts) < 4:
            await message.reply(
                ...
                "Пример: `/add_model sdxl \"Stable Diffusion XL\" https://example.com/model.safetensors \"Описание модели\"`",
                parse_mode=ParseMode.MARKDOWN
            )
            return
    ...
    try:
        ...
        if len(command_parts) < 4:
            await message.reply(
                ...
                "Пример: `/add_lora detail_xl \"Add Detail XL\" https://example.com/lora.safetensors \"Описание lora\"`",
                parse_mode=ParseMode.MARKDOWN
            )
            return
        ...
        await message.reply(
            f"✅ **Lora успешно добавлена**\n\n"
            f"Название: {lora_name}\n"
            f"URL: {lora_url}\n"
            f"Описание: {lora_description}",
            parse_mode=ParseMode.MARKDOWN
        )
    except Exception as e:
