...
class BugurtResponse(ThreadResponse):
    ...
    comments: List[str] = Field(description="Comments in 2ch style. They never have >> (double quotation marks). But they can have '>' quotation (single lesser than sign) when used to QUOTE what people saying.", min_items=2, max_items=4)
...
class GreentextResponse(ThreadResponse):
    ...
    comments: List[str] = Field(description="Comments in 4ch style on /b/ board", min_items=1, max_items=3)
