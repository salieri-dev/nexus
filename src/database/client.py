...
class DatabaseClient:
    ...
    async def connect(self, database_name: str = "nexus"):
        ...
        if not self._initialized:
            try:
                logger.info("Connecting to MongoDB", host=self.credentials.database.host, port=self.credentials.database.port)
                ...
                logger.info("Successfully connected to MongoDB", host=self.credentials.database.host, port=self.credentials.database.port)
            except Exception as e:
                logger.error("Failed to connect to MongoDB", error=str(e), host=self.credentials.database.host, port=self.credentials.database.port)
                raise
